...
system("~mdriscoll/Bin/storediffreport.pl $mount | tail -n250 | perl ./gapper.pl > $csvfile");
$pf = fopen($plotfile, "w");
$plotcmds = <<<EOT
    ...
    set yrange [0:40*1000*1000*1000]
    set key top left
    set grid

    plot "$csvfile" using 1:(\$2*-1) with lines lw 2 lc rgb "cyan" title 'gain', \
                 '' using 1:(\$3*-1) with lines lw 2 lc rgb "blue" title '10avg'
EOT;
fwrite($pf, $plotcmds);
fclose($pf);

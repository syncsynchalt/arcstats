...
system("~mdriscoll/Bin/storediffreport.pl $mount | tail -n168 | perl ./gapper.pl > $csvfile");
$pf = fopen($plotfile, "w");
$plotcmds = <<<EOT
    ...
    plot "$csvfile" using 1:(\$2*-1) with lines lw 2 lc rgb "cyan", \
                 '' using 1:(\$3*-1) with lines lw 2 lc rgb "blue"
EOT;
fwrite($pf, $plotcmds);
fclose($pf);

...
system("cat ~mdriscoll/spurge/\$(ls ~mdriscoll/spurge/ | tail -n1) | "
    ...
    . " sort | uniq -c | perl -pe 's/^ +(\d+) (.*)/$2,$1/' > $csvfile");
$pf = fopen($plotfile, "w");
$plotcmds = <<<EOT
...
#    dday=1456815600-epoch_offset
#    fit f(x) "$csvfile" using 1:2 via a,b,c,d

#    plot "$csvfile" using 1:2 with boxes lt rgb "#8B0000", f(x) lt rgb "#888888" lw 2
    plot "$csvfile" using 1:2 with boxes lt rgb "#8B0000"
EOT;
fwrite($pf, $plotcmds);
fclose($pf);
